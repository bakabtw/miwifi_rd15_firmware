...
scan_channel()
{
	...
	local cscore="`awk -F ' ' '/ '"${cchannel}"')/{print $8}' ${ACSREPORT_FILE}`"
	...
	do
		...
		score_index="`awk -F ' ' 'NR=='${line}' {print $8}' ${ACSREPORT_FILE}`"
...
get_scan_result()
{
	...
	local cscore=$(awk NR==${cchannel_line} ${ACSREPORT_FILE_NO_QUOTES} | awk '{print $8}')
